...
export function downloadOnlineSaleList(req, res, saleData) {
    try {
        ...
        res.setHeader('Content-disposition', `attachment; filename = Vsa_online_sale_data${Date.now()}.pdf`);
    ...
    // Header background
        doc.rect(margin, tableTop, pageWidth, headerHeight)
           .fill('#3498db')
           .stroke();
        ...
        const totalAmount = saleData.reduce((sum, item) => sum + parseFloat(item.price || 0), 0);
        ...
        doc.fontSize(14)
           .font('Helvetica-Bold')
           .fillColor('#2c3e50')
           .text(`Total Revenue: $${totalAmount.toFixed(2)}`, margin, summaryY, { align: 'right' });

        // Add footer with better styling
        doc.fontSize(10)
           .font('Helvetica')
           .fillColor('#95a5a6')
           .text('Generated by VSA Inventory System', 
                 0, 
                 doc.page.height - 50, 
