...
CREATE TABLE students (
  ...
  fee_structure INT,
  fee_cycle ENUM('monthly', 'quarterly', 'half-yearly', 'yearly') DEFAULT 'monthly',
  next_payment_date DATE DEFAULT NULL,
  pending_fee INT DEFAULT 0,
  transportation BOOLEAN DEFAULT FALSE,
  status ENUM('active', 'passive') DEFAULT 'active',
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  FOREIGN KEY (users_user_id) REFERENCES users(id) ON DELETE SET NULL
);
...
CREATE TABLE student_fee (
    ...
    payment_mode ENUM('admin_manual','cash','upi','card','bank') DEFAULT 'cash',
...
CREATE TABLE students_attendance (
    ...
    marked_by VARCHAR(70) DEFAULT NULL, -- FK to users/admins if needed
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (student_id) REFERENCES students(student_id) ON DELETE CASCADE,
    UNIQUE(student_id, attendance_date) -- prevent duplicates
);
...
CREATE TABLE student_achievements (
    ...
    achievement_type ENUM('competition','certificate','performance','record','other') DEFAULT 'competition',
    achievement_level ENUM('school', 'block','district', 'division', 'state','national','international') DEFAULT 'school',
...
CREATE TABLE deleted_student_achievements (
    id INT AUTO_INCREMENT PRIMARY KEY,
    student_id VARCHAR(30) NOT NULL,
    achievement_title VARCHAR(255) NOT NULL,
    achievement_type ENUM('competition','certificate','performance','record','other') DEFAULT 'competition',
...
CREATE TABLE student_fee_auto_update_log (
  id INT AUTO_INCREMENT PRIMARY KEY,
  student_id VARCHAR(30) NOT NULL,
  old_pending_fee INT DEFAULT 0, 
  new_pending_fee INT DEFAULT 0, 
  fee_added INT DEFAULT 0, 
  cycles_missed INT DEFAULT 1,    
...
CREATE TRIGGER after_achievement_delete
AFTER DELETE ON student_achievements
FOR EACH ROW
BEGIN
    INSERT INTO deleted_student_achievements (
        student_id,
        achievement_title,
        achievement_type,
        achievement_level,
        ...
        OLD.student_id,
        OLD.achievement_title,
        OLD.achievement_type,
        OLD.achievement_level,
        OLD.event_name,
        OLD.remarks,
        OLD.link,
        OLD.date_of_achievement
    );
END $$

DELIMITER ;

CREATE TABLE dashboard_stats_data (
  id INT AUTO_INCREMENT PRIMARY KEY,
  label VARCHAR(40),
  value VARCHAR(10),
  image TEXT,
...
CREATE TABLE dashboard_programs_data (
  id INT AUTO_INCREMENT PRIMARY KEY,
  title VARCHAR(100),
  category VARCHAR(20),
  description VARCHAR(256),
  price INT,
  fee_cycle VARCHAR(30),
  image TEXT,
  display_order INT DEFAULT 0,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
...
INSERT INTO dashboard_programs_data (title, category, description, price, fee_cycle, image, display_order) 
VALUES ("Beginner Program", "Beginner", "Something", 2000, "monthly",  "/images/VSA_logo.png", 1),
("Medium Program", "Medium", "Something", 5500, "quarterly",  "/images/VSA_logo.png", 2),
("Intermediate Program", "Intermediate", "Something", 16000, "half-yearly", "/images/VSA_logo.png", 3),
("Professional Program", "Professional", "Something", 29000, "yearly",  "/images/VSA_logo.png", 4);


CREATE TABLE dashboard_coaches_data (
  id INT AUTO_INCREMENT PRIMARY KEY,
  name VARCHAR(50),
...
CREATE TABLE dashboard_achievements_data (
  id INT AUTO_INCREMENT PRIMARY KEY,
  title VARCHAR(100),
...
CREATE TABLE dashboard_records_data (
  id INT AUTO_INCREMENT PRIMARY KEY,
  event VARCHAR(50),
  time VARCHAR(20),
  studentName VARCHAR(100),
  discipline ENUM('Roller Skating', 'Ice Skating', 'Roll Ball'),
  year INT,
...
CREATE TABLE dashboard_schedule_data (
  id INT AUTO_INCREMENT PRIMARY KEY,
...
CREATE TABLE dashboard_image_assets (
    id INT PRIMARY KEY AUTO_INCREMENT,
...
('logo_image', '/images/VSA_logo.png', 'logo', 'VSA Logo'),
('hero_image', '/images/hero-speed-skating.jpg', 'hero', 'Hero Banner'),
('skate_icon', '/images/skate-icon.png', 'icon', 'Skate Icon'),
('check_icon', '/images/check-icon.png', 'icon', 'Check Icon'),
('roller_skate_image', '/images/roller-skates-showcase.jpg', 'image', 'Roller Skates Showcase'),
('ice_skate_image', '/images/ice-skates-showcase.jpg', 'image', 'Ice Skates Showcase'),
('skating_banner', '/images/join-us-banner.jpg', 'banner', 'Join Us Banner'),
('roller_speed_image', '/images/roller-speed-training.jpg', 'image', 'Roller Speed Training'),
('roller_icon', '/images/roller-skate-icon.png', 'icon', 'Roller Skate Icon'),
...
CREATE INDEX idx_coaches_display_order ON dashboard_coaches_data(display_order, id);
CREATE INDEX idx_achievements_display_order ON dashboard_achievements_data(display_order, id);
CREATE INDEX idx_records_featured ON dashboard_records_data(is_featured, year, id);
CREATE INDEX idx_schedule_day ON dashboard_schedule_data(day, discipline);
...
-- Insert Programs
INSERT INTO dashboard_programs_data (title, category, description, price, fee_cycle, image, display_order) VALUES
('Speed Roller Skating Junior', 'Roller', 'Master the fundamentals of speed roller skating with professional instruction.', 2000, 'month', '/images/speed-roller-program.jpg', 1),
('Speed Speed Skating', 'Roller', 'Develop lightning-fast techniques on ice with Olympic-level coaching.', 5500, '3 months', '/images/ice-speed-program.jpg', 2),
('Competitive Training', 'Elite', 'Advanced program for competitive athletes seeking championship levels.', 10000, '6 months', '/images/competitive-program.jpg', 3)
('Professional Training', 'Master', 'Advanced program for master athletes seeking championship levels.', 16000, '11 months', '/images/competitive-program.jpg', 4);

-- Insert Coaches
INSERT INTO dashboard_coaches_data (name, specialization, experience, image, display_order) VALUES
('Vaibhav Agrawal', 'Speed Roller Skating Coach', '22+ years competitive experience', '/images/coaches-icon.jpg', 1),
('Visheshta Singh', 'Roller & Ice Speed Skating Expert', 'Former international athlete', '/images/hero-speed-skating.jpg', 2),
('Harsh Kasuadhan', 'Youth Development Coach', '3+ years teaching experience', '/images/HarshPic.jpg', 3);

-- Insert Achievements
INSERT INTO dashboard_achievements_data (title, description, images, display_order) VALUES
('National Champions', '30+ students won national titles', '/images/national_players.jpg', 1),
('International Qualifiers', '10 students qualified for international', '/images/ice-speed-program.jpg', 2),
('Record Holders', '50+ regional speed records', '/images/record-icon.jpg', 3),
('Academy Awards', 'Top skating academy 22 years running', '/images/ice-speed-program.jpg', 4);

-- Insert Records
INSERT INTO dashboard_records_data (event, time, studentName, discipline, year, is_featured) VALUES
('500m Sprint', '45.23s', 'Rahul Sharma', 'Roller Skating', 2024, TRUE),
('1000m', '1:32.45', 'Priya Patel', 'Roller Skating', 2024, TRUE),
('500m', '38.42s', 'Arjun Singh', 'Ice Skating', 2024, TRUE),
('1500m', '1:58.34', 'Sneha Gupta', 'Ice Skating', 2024, TRUE);

-- Insert Schedule
INSERT INTO dashboard_schedule_data (day, level, time, duration, discipline) VALUES
('Monday', 'Beginner', '6:00 AM', '2 hours', 'Roller Skating'),
('Tuesday', 'Intermediate', '7:00 AM', '2.5 hours', 'Roller Skating'),
('Monday', 'Beginner', '8:00 AM', '2 hours', 'Ice Skating'),
('Tuesday', 'Intermediate', '7:30 AM', '2.5 hours', 'Ice Skating'),
('Wednesday', 'Beginner', '6:00 AM', '2 hours', 'Roller Skating'),
('Thursday', 'Intermediate', '7:00 AM', '2.5 hours', 'Roller Skating'),
('Wednesday', 'Beginner', '8:00 AM', '2 hours', 'Ice Skating'),
('Thursday', 'Intermediate', '7:30 AM', '2.5 hours', 'Ice Skating'),
('Friday', 'Beginner', '6:00 AM', '2 hours', 'Roller Skating'),
('Saturday', 'Intermediate', '7:00 AM', '2.5 hours', 'Roller Skating'),
('Friday', 'Beginner', '8:00 AM', '2 hours', 'Ice Skating'),
('Saturday', 'Intermediate', '7:30 AM', '2.5 hours', 'Ice Skating'),
('Sunday', 'Beginner', '6:00 AM', '2 hours', 'Roller Skating'),
('Sunday', 'Intermediate', '7:30 AM', '2.5 hours', 'Ice Skating');


CREATE TABLE skating_events_and_tours (
  id INT AUTO_INCREMENT PRIMARY KEY,
  title VARCHAR(50),
  description VARCHAR(256),
  event_date TIMESTAMP,
  competition_category ENUM('Roller Speed Skating', 'Roll Ball', 'Ice Skating', 'Record', 'Camp'),
  competition_level ENUM('school', 'block','district', 'division', 'state','national','international', 'record', 'RSFI', 'SGFI'),
...
INSERT INTO skating_events_and_tours 
(title, description, event_date, competition_category, competition_level, location, tour_fees)
VALUES
('Winter Speed Championship',
 'Annual winter championship featuring 200m, 500m, and 1000m races. Open to all age categories with separate divisions for beginners and advanced skaters.',
 '2025-12-15 00:00:00',
...
('New Year Training Camp',
 'Intensive 5-day training camp focused on technique refinement and speed building. Perfect for athletes preparing for upcoming competitions.',
 '2025-12-28 00:00:00',
 'Camp',
 'district',
 'Academy Facility',
...
('Maharashtra State Trials',
 'Official state-level trials for selecting athletes to represent Maharashtra at the National Speed Skating Championship.',
 '2026-01-20 00:00:00',
 'Roller Speed Skating',
 'state',
 'Mumbai',
...
('Youth Development Workshop',
 'Free workshop for young skaters aged 8-14. Learn fundamental techniques, safety protocols, and experience the thrill of speed skating.',
 '2026-02-14 00:00:00',
 'Camp',
 'school',
 'Rama Krishna',
...
INSERT INTO coaches 
(coach_name, image, experience, description, specialization, achievements, certifications, status)
VALUES
('Vaibhav Agrawal', '/images/coaches/vaibhav_sir.jpg', '20+ years',
 'Former national speed skater with extensive experience coaching elite athletes.',
 'Roller Speed Skating',
 'Trained 5 national-level skaters; Gold medalist.',
 'RSFI Certified Coach, First Aid Certified',
 'active'),

('Visheshta Singh', '/images/coaches/sneha_deshmukh.jpg', '10 years',
 'Dedicated coach focusing on youth development and professional speed training.',
 'Ice Skating',
 'Developed many champions under her guidance.',
 'ISI',
 'active'),

('Harsh Kasuadhan', '/images/coaches/HarshPic.jpg', '150 years',
 'Expert in endurance and sprint skating with focus on biomechanics and strength.',
 'Roller Speed Skating',
 'Coach of MP State Team 2035; Former National Gold medalist.',
 'RSFI Certified, Sports Science Diploma',
 'active');
...
CREATE TABLE academy_achievements (
  ...
  description TEXT,
  event_date DATE,
  total_medals_won INT DEFAULT 0,
...
CREATE TABLE academy_achievements_images (
  id INT AUTO_INCREMENT PRIMARY KEY,
  academy_achievement_id INT NOT NULL,
  images_url TEXT,
  FOREIGN KEY (academy_achievement_id) REFERENCES academy_achievements(id) ON DELETE CASCADE
);

INSERT INTO academy_achievements (
  title, description, event_date, total_medals_won, gold_medals, silver_medals, bronze_medals,
  competition_name, competition_category, competition_level, location, banner_image
) VALUES (
  'Maharashtra State Roller Speed Skating Championship 2024',
  'Our academy team delivered an outstanding performance, securing multiple medals across different age groups.',
  '2024-02-15', 12, 5, 4, 3,
  'State Roller Skating Championship 2024',
  'Roller Speed Skating',
  'state',
  'Pune, Maharashtra',
  '/images/achievements/1744981361378-Suchi_Image.jpeg'
);

INSERT INTO academy_achievements_images (academy_achievement_id, images_url) VALUES
  (1, '/images/achievements/1744981361378-Suchi_Image.jpeg'),
  (1, '/images/achievements/1744981361378-Suchi_Image.jpeg'),
  (1, '/images/achievements/1744981361378-Suchi_Image.jpeg');
...
INSERT INTO academy_achievements (
  ...
  'National Roll Ball Championship 2023',
  'Our players showcased great teamwork and speed at the National Roll Ball event.',
  '2023-11-05', 8, 3, 3, 2,
  'National Roll Ball Championship 2023',
  'Roll Ball',
  'national',
  'Jaipur, Rajasthan',
  '/images/achievements/VisheshtaPic.jpg'
);

INSERT INTO academy_achievements_images (academy_achievement_id, images_url) VALUES
  (2, '/images/achievements/VisheshtaPic.jpg'),
  (2, '/images/achievements/VisheshtaPic.jpg');
...
INSERT INTO academy_achievements (
  ...
  'Winter Ice Skating Invitational 2024',
  'Our skaters displayed excellent balance and control during the national-level ice skating championship.',
  '2024-01-20', 6, 2, 3, 1,
  'Winter Ice Skating Invitational 2024',
  'Ice Skating',
  'national',
  'Gulmarg, Jammu & Kashmir',
  '/images/achievements/WhatsApp Image 2024-07-29 at 16.57.35.jpeg'
);

INSERT INTO academy_achievements_images (academy_achievement_id, images_url) VALUES
  (3, '/images/achievements/WhatsApp Image 2024-07-29 at 16.57.35.jpeg'),
  (3, '/images/achievements/WhatsApp Image 2024-07-29 at 16.57.35.jpeg');
...
INSERT INTO academy_achievements (
  ...
  'Limca Book of Records - Longest Skating Marathon',
  'Our academy entered the Limca Book of Records for the longest continuous skating relay performed by students.',
  '2023-12-10', 0, 0, 0, 0,
  'Limca Record Event 2023',
  'Record',
  'record',
  'Mumbai, Maharashtra',
  '/images/achievements/WhatsApp Image 2024-07-29 at 16.57.54.jpeg'
);

INSERT INTO academy_achievements_images (academy_achievement_id, images_url) VALUES
  (4, '/images/achievements/WhatsApp Image 2024-07-29 at 16.57.54.jpeg'),
  (4, '/images/achievements/WhatsApp Image 2024-07-29 at 16.57.54.jpeg'),
  (4, '/images/achievements/WhatsApp Image 2024-07-29 at 16.57.54.jpeg');
...
INSERT INTO academy_achievements (
  ...
  'Summer Skating Training Camp 2024',
  'A week-long intensive training camp focused on building speed, endurance, and discipline among young skaters.',
  '2024-05-05', 0, 0, 0, 0,
  'Swasth Skating Academy Summer Camp 2024',
  'Camp',
  'state',
  'Nagpur, Maharashtra',
  '/images/achievements/WhatsApp Image 2024-08-06 at 12.24.34.jpeg'
);

INSERT INTO academy_achievements_images (academy_achievement_id, images_url) VALUES
  (5, '/images/achievements/WhatsApp Image 2024-08-06 at 12.24.34.jpeg'),
  (5, '/images/achievements/WhatsApp Image 2024-08-06 at 12.24.34.jpeg'),
  (5, '/images/achievements/WhatsApp Image 2024-08-06 at 12.24.34.jpeg');


CREATE TABLE item_sold_offline (
  id INT PRIMARY KEY AUTO_INCREMENT,
...
CREATE TABLE item_sold_offline_items (
  id INT PRIMARY KEY AUTO_INCREMENT,
  item_sold_offline_id INT NOT NULL,
