      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-20">
          ...
          <div class="flex items-center space-x-4">
            <div
              class="w-12 h-12 bg-gradient-to-r from-cyan-400 to-purple-500 rounded-xl flex items-center justify-center shadow-lg">
          ...
          <div class="hidden lg:flex items-center space-x-6">
            <a v-if="hasPermission('show_order_status')" href="/ordersStatus"
            ...
            <a v-else href="/login"
              class="flex items-center space-x-2 bg-gradient-to-r from-green-500 to-blue-500 text-white px-6 py-2 rounded-xl hover:from-green-600 hover:to-blue-600 transition-all duration-200 shadow-lg hover:shadow-xl">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div v-if="filteredAdminOperations.length > 0" class="mb-12">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div v-for="operation in filteredAdminOperations" :key="operation.id"
            ...
            <div class="relative p-6">
              <div class="flex items-center justify-between mb-4">
                <div class="p-3 bg-black rounded-xl shadow-lg text-white">
                  <svg class="w-6 h-6 text-white" :class="operation.icon" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
                    <path v-if="operation.id === 1" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    <path v-else-if="operation.id === 2" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      d="M3 3h2l.4 2M7 13h10l4-8H5.4m0 0L7 3H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17M17 13v6a2 2 0 01-2 2H9a2 2 0 01-2-2v-6m8 0V9a2 2 0 00-2-2H9a2 2 0 00-2 2v4.01" />
                    <path v-else-if="operation.id === 3" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                    <path v-else-if="operation.id === 4" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" />
                    <path v-else-if="operation.id === 5" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
                    <path v-else-if="operation.id === 6" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                    <path v-else-if="operation.id === 7" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              ...
              <div v-if="operation.id === 1 || operation.id === 7 || operation.id === 8">
                ...
                <a :href="operation.link"
                  ...
                  <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path v-if="operation.buttonIcon.includes('plus')" stroke-linecap="round" stroke-linejoin="round"
              ...
              <div v-else class="space-y-2">
                <!-- PDF Download Button -->
                <button @click="downloadFile(operation.id, operation.endpoints.pdf, 'pdf')"
                  :disabled="loadingStates[operation.id + '_pdf']"
                  class="inline-flex items-center justify-center w-full px-4 py-3 bg-white text-black font-medium rounded-xl hover:bg-gray-800 hover:text-white transform hover:scale-105 transition-all duration-200 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none">
                  <svg v-if="!loadingStates[operation.id + '_pdf']" class="w-5 h-5 mr-2" fill="none"
                  ...
                  {{ loadingStates[operation.id + '_pdf'] ? 'Downloading...' : 'Download PDF' }}
                </button>

                <!-- CSV Download Button -->
                <button @click="downloadFile(operation.id, operation.endpoints.csv, 'csv')"
                  :disabled="loadingStates[operation.id + '_csv']"
                  ...
                  <svg v-if="!loadingStates[operation.id + '_csv']" class="w-5 h-5 mr-2" fill="none"
                    stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  ...
                  {{ loadingStates[operation.id + '_csv'] ? 'Downloading...' : 'Download CSV' }}
                </button>
              </div>
            </div>
          </div>
      ...
      <div v-if="filteredStudentManagement.length > 0" class="mb-12">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div v-for="management in filteredStudentManagement" :key="management.id"
            class="group relative overflow-hidden rounded-2xl bg-white/10 backdrop-blur-lg border border-white/20 hover:bg-white/20 transition-all duration-300 hover:scale-105 hover:shadow-2xl">
            <div
              class="absolute inset-0 bg-gradient-to-br from-green-500/20 to-blue-500/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            </div>
            ...
            <div class="relative p-6">
              <div class="flex items-center justify-between mb-4">
                <div class="p-3 bg-gradient-to-r from-green-500 to-blue-500 rounded-xl shadow-lg">
                  <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path v-if="management.id === 1" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" />
                    <path v-else-if="management.id === 2" stroke-linecap="round" stroke-linejoin="round"
                      stroke-width="2"
                      d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" />
                    <path v-else-if="management.id === 3" stroke-linecap="round" stroke-linejoin="round"
                      stroke-width="2"
                      d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                    <path v-else-if="management.id === 4" stroke-linecap="round" stroke-linejoin="round"
                      stroke-width="2"
                      d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z" />
                    <path v-else-if="management.id === 5" stroke-linecap="round" stroke-linejoin="round"
                      stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z M8 12l4 4 8-8" />
                    <path v-else-if="management.id === 6" stroke-linecap="round" stroke-linejoin="round"
                      stroke-width="2"
                      d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2M9 3v2m6-2v2M9 19h1m4 0h1" />
                  </svg>
                </div>
                <div class="text-right">
              ...
              <a :href="management.link"
                ...
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path v-if="management.buttonIcon.includes('plus')" stroke-linecap="round" stroke-linejoin="round"
                    stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                  <path v-else-if="management.buttonIcon.includes('check')" stroke-linecap="round"
                    stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  <path v-else-if="management.buttonIcon.includes('cogs')" stroke-linecap="round"
                    stroke-linejoin="round" stroke-width="2"
                    d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
                  <path v-else-if="management.buttonIcon.includes('search')" stroke-linecap="round"
      ...
      <div v-if="filteredAdminOperations.length === 0 && filteredStudentManagement.length === 0"
        ...
        <div class="bg-white/10 backdrop-blur-lg rounded-2xl p-8 max-w-md mx-auto">
          ...
          <p class="text-gray-300">You don't have permission to access any admin features. Please contact your
            administrator.</p>
        </div>
      </div>
    </div>
    ...
    <div class="fixed bottom-4 right-4 z-50">
      <div v-if="showToast"
        ...
        <div class="flex items-center p-4"
          :style="{ background: `linear-gradient(135deg, ${toastHeaderColor}, ${toastHeaderColor}dd)` }">
    ...
    return {
      ...
      adminOperations: [
        ...
        },
        {
          ...
          description: 'Download sales data in PDF or CSV format.',
          endpoints: {
            pdf: '/vsa/download-online-sale-list',
            csv: '/vsa/download-online-sale-csv'
          },
        ...
        },
        {
          ...
          description: 'Download the complete list of offline customers in PDF OR CSV format.',
          endpoint: '',
          endpoints: {
            pdf: '/vsa/download-offline-user-data',
            csv: '/vsa/download-offline-user-data-csv'
          },
          buttonIcon: 'fas fa-file-download',
          buttonText: {
            pdf: 'Download PDF',
        ...
        },
        {
          ...
          icon: 'fas fa-users',
          description: 'Download the complete list of online users in PDF OR CSV format.',
          endpoints: {
            pdf: '/vsa/download-online-user-data',
            csv: '/vsa/download-online-user-data-csv'
          },
          buttonIcon: 'fas fa-file-download',
          buttonText: {
            pdf: 'Download PDF',
        ...
        },
        {
          ...
          description: 'Download the complete list of available stock in PDF OR CSV format.',
          endpoints: {
            pdf: '/vsa/download-available-stock',
            csv: '/vsa/download-available-stock-csv'
          },
          buttonIcon: 'fas fa-file-download',
          buttonText: {
            pdf: 'Download PDF',
  ...
  methods: {
    ...
    async downloadFile(operationId, endpoint, fileType) {
      ...
      try {
        ...
        a.download = `${this.getFileName(operationId)}-${timestamp}.${extension}`;
    ...
    getFileName(operationId) {
      const names = {
        2: 'online-sales-report',
        3: 'offline-user-list',
        4: 'online-users-list',
        5: 'available-stock-inventory',
        6: 'offline-sales-report'
      };
      return names[operationId] || 'download';
    }
  }
}
</script>
